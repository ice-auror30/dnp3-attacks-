				...
				if(payload[12] == '\x81'):  #and payload[15] == '\x20' and payload[16] == '\x05'
					...
					if(len(payload) > 23):
						...
						if((value > 1 or value < -1) and (value2 > 1 or value2 < -1)and (value3 > 1 or value3 < -1) and value < 10000 and value2 < 10000 and value3 < 100000):
							...
							pkt[Raw].load=payload[:23]+value[:3]+crc+value[3]+payload[29:32]+value2+payload[36:39]+value3+crc+payload[46:]
							...
							pkt[Raw].load=payload[:23]+value[:3]+crc1+value[3]+payload[29:32]+value2+payload[36:39]+value3+crc2+payload[46:]
							print pkt.show()
							del pkt[IP].chksum
							del pkt[TCP].chksum
							del pkt[IP].len
